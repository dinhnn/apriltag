...
JNIEXPORT jboolean JNICALL Java_edu_umich_eecs_april_apriltag_ApriltagNative_homography_1project
        (JNIEnv *env, jclass cls, jfloatArray coordinates)
{
    int len = (*env)->GetArrayLength(env,coordinates);
    int known_nodes = 0;
    zarray_t *correspondences = zarray_create(sizeof(float[4]));
    zarray_ensure_capacity(correspondences,known_nodes);
